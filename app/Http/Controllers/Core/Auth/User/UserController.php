...
class UserController extends Controller
{
    ...
    public function index()
    {
        return (new AppUserFilter(
            $this->service
                ->filters($this->filter)
                //->select(['id', 'first_name', 'last_name', 'email', 'created_by', 'status_id', 'created_at', 'document'])
                ->with('roles:id,name,is_admin,is_default,type_id', 'status', 'profilePicture', 'account')
                ->latest()
        ))->filter()
